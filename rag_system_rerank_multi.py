...
class RAGSystem:
    ...
    def answer_question(self, question: str, top_k: int = 3) -> Dict:
        ...
        merged_results = sorted(merged_results, key=lambda x: x[1], reverse=True)[:k * 2]
...
def main():
    ...
    if os.path.exists(f"rag_index_{chunk_size}/faiss_index.idx"):
        print("Loading existing RAG system...")
        rag.load_system(f"rag_index_{chunk_size}")
    else:
        print("Building new RAG system...")
        rag.build_system(crawled_data_path)
        rag.save_system(f"rag_index_{chunk_size}")
    ...
    evaluation_results = rag.evaluate_system(
        qa_file_path="dataset/filtered_eval_dataset.jsonl",  # Your QA pairs file
        top_k=3,
